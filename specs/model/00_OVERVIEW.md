...
- Phase 3: pool-level predicted-vs-realized analysis (scatter + R²/RMSE by pool bucket scheme) for NN vs logit vs empirical at the 12-month horizon, at a minimum of 3 window anchors (ideally all 11); plus the **economic translation** (`03 §5`): cashflow engine → CPR/WAL/price errors per pool × model × anchor (T5.1/F5.2) — the beyond-replication contribution exercise. Memo with contributions paragraph.
