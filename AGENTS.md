# AGENTS.md — Dissertation: Asset Loans Default Risk
...
- `src/floan/pipeline/` — the data pipeline code (COMPLETE). **Its `AGENTS.md` is binding for
...
- `specs/pipeline/` — the pipeline specification (executed): `00_OVERVIEW.md` →
  `01_SCHEMA.md` → `02_PIPELINE_STAGES.md` → `03_CODEX_TASKS.md`;
  `CODEX_WORKFLOW.md` is the operator guide.
- `specs/model/` — the analysis and modelling specification:
  ...
  the model-/horizon-/calibration-agnostic pricing engine (tasks M20–M27).
...
- `artifacts/` — the final dissertation and compact result artifacts; large binaries use Git LFS.
...
## Critical invariants (apply everywhere, full detail in src/floan/pipeline/AGENTS.md)
...
4. Stages are idempotent and per-quarter; execute `03_CODEX_TASKS.md` one task
   at a time and verify each task's **Accept** criteria before advancing.

## Working style

Follow the Karpathy guidelines (condensed in `src/floan/pipeline/AGENTS.md`): state
assumptions before coding, simplest solution that works, surgical diffs only,
verify against explicit success criteria.

## Git

**Never** append a `Co-Authored-By` trailer naming an AI assistant to commit
