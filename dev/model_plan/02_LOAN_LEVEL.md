...
- **Continuous (standardized; log where flagged):** current/original rate, **incentive** (`01_EDA §4`), orig & current UPB (log), loan age, remaining months, original term, LTV, CLTV, DTI, `fico_orig`, co-borrower FICO, MI %, unscheduled principal.
...
1. **Train pool** (thinned, weighted): assemble via DuckDB from the panel + `mkt_rate` join, origin-state filter, all years. Keep ALL rows with `state_next ≠ current` or `state_t ≠ current`; keep `current→current` rows with probability `p_keep` (config; start 0.05–0.10). Attach `weight = 1/p_keep` (1.0 otherwise). The loss is weight-averaged so estimated probabilities remain unbiased. Window k trains on `train_pool WHERE label_month ≤ Dec(k−2)`.
