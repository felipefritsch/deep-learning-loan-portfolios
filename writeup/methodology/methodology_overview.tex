...
% MD1-MD4) refer to specs/model/01_EDA.md, 02_LOAN_LEVEL.md,
...
\normalsize (companion document to the \code{specs/model} specification)}
...
\subsection{Contributions beyond replication}\label{sec:contributions}

The dissertation deliberately adopts a replication \emph{design} --- the cleanest way to attribute findings to data and regime rather than to modelling idiosyncrasies --- but four elements are original:

\begin{enumerate}[itemsep=2pt]
\item \textbf{A prime, conforming testbed.} The paper's evidence comes from private-label, subprime-heavy data (1995--2014). This study asks whether the deep-learning advantage survives in the clean GSE credit box, where events are rarer and relationships plausibly tamer. An early answer is already in hand: the architecture search at full scale selects \emph{three} hidden layers over the paper's five --- the nonlinearity premium is real but structurally simpler in prime credit, an empirical statement about \emph{where} model complexity pays.
\item \textbf{Eleven test years the literature has not examined,} 2015--2025, including the COVID forbearance episode, the 2020--21 refinancing wave, and the 2022--23 rate shock --- regimes that post-date the paper's sample entirely.
\item \textbf{A stricter evaluation protocol whose output is itself a finding.} Replacing the paper's single temporal split with an eleven-window rolling backtest produces a \emph{time series of the value of nonlinearity} (\artifact{Table B}; its pool-level mirror by anchor year): in which market regimes does the deep model's edge widen, compress, or invert? The paper's design cannot ask this question.
\item \textbf{An economic translation for the pool level} (Section~\ref{sec:pool}): predicted and realised pool prepayments are converted into each pool's \emph{prepayment-speed path} --- the model's month-by-month forecast of what fraction of the pool's remaining principal prepays, in the weighting that pool cashflows actually obey --- run through a simple cashflow engine, and compared as errors in \emph{cashflow timing and price} (\artifact{T5.1, F5.2}). This restates the model comparison in the units an investor in mortgage pools actually uses --- ``the linear model misprices high-incentive pools by $x$ per 100 of face value'' --- rather than in log-likelihood alone.
\end{enumerate}
...
The class structure is extremely imbalanced: $\state{current}\!\to\!\state{current}$ accounts for the overwhelming majority of the $\sim$3.3 billion loan-months, while $\state{dpd\_90plus}\!\to\!\state{REO}$ events are vanishingly rare \artifact{T1.3, T2.1}; measured shares in Table~\ref{tab:stateshares}. Section~\ref{sec:thinning} explains how training exploits this without biasing the estimated probabilities.
...
\subsection{Phase 1 results (executed; memo \code{writeup/memos/01\_eda.md})}\label{sec:edaresults}

Phase 1 is complete; this subsection records what the programme found. All empirical rates carry Wilson 95\% intervals and every number regenerates from \code{src/floan/analysis/}.

\paragraph{Scope.} The assembled panel is the complete release: \textbf{3{,}312{,}456{,}883 loan-months across 57{,}562{,}668 loans}, reporting months 2000-01 through 2025-12, in 104 acquisition-vintage partitions \artifact{T1.1}.

\paragraph{Class imbalance.} The state distribution \artifact{T1.3} confirms --- and sharpens --- the design assumptions of Section~\ref{sec:thinning}:
...
\midrule
\state{current}    & 96.28\,\% & \state{dpd\_60}      & 0.31\,\% \\
\state{prepaid}    & 1.25\,\%  & \state{REO}          & 0.014\,\% \\
\state{dpd\_30}    & 1.15\,\%  & \state{foreclosure}  & 0.0067\,\% \\
\state{dpd\_90plus}& 0.98\,\%  & & \\
\bottomrule
\end{tabular}
\caption{Loan-month shares by state \artifact{T1.3}. The 96\% \state{current} mass motivates the thinning-with-importance-weights scheme of \eqref{eq:weightednll}; credit-event terminals at $\sim$1-in-10{,}000 motivate the Laplace smoothing of \eqref{eq:empirical}.}
\label{tab:stateshares}
\end{table}

Pooled over time \artifact{T2.1}, the monthly rates out of \state{current} are $1.27\%$ to \state{prepaid} and $0.64\%$ to \state{dpd\_30} --- but the pooling hides the point: the monthly series \artifact{F2.2} shows the 2003 and 2020--21 refinancing waves, the 2008--11 default wave, and the sharp 2022--23 prepayment collapse. These regime shifts, not any single structural break, are the empirical justification for the rolling backtest of Section~\ref{sec:design}.

\paragraph{Nonlinearity (single covariates).} The three hazard curves deliver the predicted shapes, now with magnitudes:
\begin{itemize}[itemsep=1pt]
\item \artifact{F3.1} \emph{seasoning hump}: $\state{current}\to\state{prepaid}$ rises from $0.27\%$/mo at origination to $1.56\%$/mo at $\approx$17 months, then burns out to $1.11\%$/mo by $\approx$52 months (with a secondary late-life rise to $\sim$1.65\%/mo among 15-year+ survivors) --- strongly non-monotone;
\item \artifact{F3.2} \emph{refinancing S-curve}: with $\code{incentive}=r^{\mathrm{cur}}-\code{pmms30}(t)$, prepayment is flat at $0.4$--$0.6\%$/mo out-of-the-money, turns sharply through zero, peaks at $2.71\%$/mo near $+1.5$\,pp and burns out to $2.16\%$/mo by $+2.7$\,pp --- a $\sim$7$\times$ range with both an inflection and a turning point, reproducing the paper's flagship nonlinearity on this cleaner credit box;
\item \artifact{F3.3} \emph{convex FICO decay}: $\state{current}\to\state{dpd\_30}$ falls $\sim$24$\times$, from $3.06\%$/mo at FICO $\approx$623 to $0.13\%$/mo at FICO $\approx$815, with the low-FICO half $\sim$3.7$\times$ steeper than the high-FICO half.
\end{itemize}

\paragraph{Interactions (the case against additivity).} The headline exhibit \artifact{F3.4} puts numbers on the misspecification argument: on a FICO$\times$LTV grid the $\state{current}\to\state{dpd\_30plus}$ rate spans $0.11\%$--$2.69\%$/mo, and the \emph{additive FICO gradient} (worst-minus-best FICO rate) grows from $1.73$\,pp at low LTV to $2.48$\,pp at high LTV ($+43\%$) --- equivalently, the LTV effect is $\sim$8.5$\times$ larger for low-FICO than high-FICO loans. An additive model forces this gradient to be constant; the violation sits exactly in the low-FICO/high-LTV corner that drives credit losses. In the prepayment channel, \artifact{F3.5} shows the in-the-money refinancing response \emph{rising} with credit quality ($2.36\%$/mo in the low-FICO tercile vs.\ $3.01\%$/mo in the high) --- credit-constrained borrowers cannot exercise the option, so the incentive \emph{slope} itself depends on FICO. Finally \artifact{F3.6}, holding loan age fixed at 12--36 months, isolates a clean cohort effect: the 2007 vintage's delinquency hazard of $1.68\%$/mo is $\times$3.6 the calm-cohort level of $0.47\%$/mo --- vintage is a feature in its own right.

\paragraph{Validation and sanity checks.} The panel-derived market-rate proxy tracks the downloaded PMMS series with correlation $0.991$, RMSE $0.198$\,pp and MAE $0.147$\,pp \artifact{F4.1} --- so the PMMS-based incentive is sound and the proxy is a credible robustness alternative. The shard checks of Section~\ref{sec:shards} pass with room to spare: rows and exact distinct loans per shard are uniform to coefficients of variation $0.26\%$ and $0.21\%$ respectively \artifact{F5.1}, and per-shard vintage composition matches the panel marginal (CV $\le 2.3\%$ for every vintage above $0.5\%$ of the book) \artifact{F5.2} --- the shards are not vintage-clustered, closing out the failure mode this design exists to prevent.\footnote{A methodological footnote worth keeping: \code{approx\_count\_distinct} grouped over the 256 shards reported a spurious $\sim$30\% spread that the exact \code{count(DISTINCT)} flatly contradicts; F5.1 therefore uses exact counts. Approximate aggregates should not be trusted for uniformity checks.}

\paragraph{The hypothesis carried into Phase 2.} Transition risk depends on covariates nonlinearly (F3.1--F3.3) and interactively (F3.4--F3.6). A linear-additive multinomial logit should therefore underfit, most severely on \emph{prepayment} --- the channel combining the sharpest single-variable nonlinearity (the S-curve) with a first-order interaction with credit quality. Phase 2 tests precisely this, and the prepayment-conditional NLL of \artifact{Table B} is the designated place to look for the deep model's largest edge.
...
Training on every $\state{current}\!\to\!\state{current}$ row is wasteful: \state{current} is $96.3\%$ of loan-months (Table~\ref{tab:stateshares}) and such rows are individually nearly uninformative. The export therefore keeps every row whose origin or destination is \emph{not} \state{current}, but retains $\state{current}\!\to\!\state{current}$ rows only with probability $p$ (e.g.\ $p = 0.05$--$0.10$), attaching the weight $w_i = 1/p$ to kept rows (and $w_i=1$ otherwise). The training objective is the weighted negative log-likelihood
\begin{equation}
...
\end{equation}
the Gaussian being the standard approximation to the Poisson--binomial distribution of a sum of independent, non-identical Bernoullis (conditional independence given covariates is the maintained assumption, inherited from the paper). The comparison is then direct and visual: predicted vs.\ realised counts per pool \artifact{F4.1}, $R^2$/RMSE by model, outcome and anchor \artifact{T4.2} --- and signed errors across characteristic buckets \artifact{F4.3}, which localise \emph{where} the linear model breaks (the prediction: high-incentive cells). Interval coverage (the share of random pools whose realised count falls inside the model's 90\% band) tests \eqref{eq:poolcount}'s distributional claim, not just its mean. Run at every window anchor, the by-anchor accuracy table doubles as the regime exhibit of contribution 3 (Section~\ref{sec:contributions}).

\subsection{Economic translation: prepayment speed, cashflow timing, and price}\label{sec:econtranslation}

The final step --- contribution 4 --- converts counts into valuation units; each term is defined as it first appears. The roll-forward already yields each loan's monthly prepayment probability. Averaging these with each loan weighted by its \emph{unpaid principal balance} gives the pool's \textbf{prepayment-speed path}: the model's month-by-month forecast of what fraction of the pool's remaining principal prepays --- the units and weighting that pool cashflows actually obey, since a large loan returns proportionally more principal than a small one. (Market shorthand, used hereafter: the monthly fraction is the \emph{single monthly mortality}, SMM; annualised it is the \emph{conditional prepayment rate}, CPR --- the speed quote of mortgage markets.) The realised path is computed identically from the panel. A small deterministic cashflow engine then turns any speed path into pool cashflows, treating the pool as a \emph{pass-through} (investors simply receive the pool's interest and principal as it is paid): scheduled amortisation plus forecast prepayments, month by month, with the final month's speed held constant beyond the 12-month horizon. From the cashflows follow two valuation summaries: \textbf{weighted-average life} --- the average time a dollar of principal remains outstanding, i.e.\ the \emph{timing} of money returned --- and \textbf{price}, the discounted value of the cashflows, with every model's cashflows discounted at the same fixed rate so that any price difference comes from the prepayment forecasts alone. Comparing model-implied to realised-path valuations gives, per pool $\times$ model $\times$ anchor: a speed error (percentage points of CPR), a timing error (months of weighted-average life), and a price error (per 100 of face value) \artifact{T5.1}, with the headline being the ensemble-vs-logit reduction in mean absolute price error, and the bucket-level map \artifact{F5.2} expected to concentrate the linear model's mispricing in high-incentive cells --- the pricing mirror of the EDA's S-curve. The engine is unit-tested against two closed forms (with no prepayment a pool is a textbook annuity; a constant speed has a closed-form survival schedule). Stated scope: with the macroeconomic environment frozen at the anchor and a common fixed discount rate, the exercise isolates the \emph{prepayment-model component} of valuation error --- deliberately simpler than the market's full option-adjusted-spread machinery.

\paragraph{Why pricing rather than portfolio sorts.} \citet{sirignano2021} demonstrate economic significance with a \emph{portfolio-sort} exercise: rank pools by predicted prepayment risk, form best- and worst-decile portfolios, and compare their realised outcomes. This study runs the pricing exercise above instead, and the choice deserves a plain explanation. The two designs test different properties of the same predictions. A sort uses only the \emph{ordering} of pools --- it is blind to probability levels, so a model whose hazards are uniformly twice the truth sorts exactly as well as a perfectly calibrated one. Pricing uses the \emph{levels}: weighted-average life and price depend on the actual magnitude of the predicted prepayment path. Which test is more informative depends on which property of the model one trusts and which one's user needs. Ranking-only evidence is the natural choice where levels are inherently untrustworthy (the classic asset-pricing setting); here, by contrast, the transition probabilities are explicitly calibration-checked (Section~\ref{sec:eval}), so a ranking-only exercise would discard the model's strongest property --- and the practical consumer of a pool-level mortgage model (pricing a pool, marking it to market, hedging a position in it) consumes levels, not ranks. Running both would add little: each is a deterministic re-presentation of the same per-loan probabilities $q_i$, their respective properties are already tested more rigorously upstream (discrimination by the AUC tables, calibration by the reliability diagrams), and the bucket-level price-error map \artifact{F5.2} conveys what a sort would dramatise --- \emph{where} the linear model goes wrong --- while also conveying by \emph{how much}, in the units the market trades. The sort exercise is therefore noted as future work rather than duplicated.
...
\artifact{Pool F4.1, T4.2, F4.3} & predicted-vs-realised scatter; $R^2$/RMSE table (by anchor); bucket error map & M14 \\
\artifact{T5.1, F5.2} & speed/timing/price-error table; price-error bucket map (economic translation) & M15 \\
\bottomrule
\end{tabular}
\caption{Map from artifact codes used in this document to the plan tasks that produce them. Task definitions and acceptance criteria: \code{specs/model/04\_TASKS.md}; EDA codes: \code{01\_EDA.md}; model/evaluation codes: \code{02\_LOAN\_LEVEL.md}; pool codes: \code{03\_POOL\_LEVEL.md}; macro: \code{05\_MACRO\_DATA.md}.}
