"""M27a GPU run — train+score all 11 windows (seed 0) + seeds 1,2 on the 5 key windows.
Reuses seq_model/seq_train/evaluate/sequence/torch_common. Resumable per (k,seed). Serial on cuda.
Artifacts: RESULTS/k{k}_s{seed}.npz (test probs/y/origin) + .json (metrics)."""
...
def run_one(k: int, seed: int):
    npz = RESULTS / f"k{k}_s{seed}.npz"
    js = RESULTS / f"k{k}_s{seed}.json"
    if npz.exists() and js.exists():
        print(f"[k{k} s{seed}] SKIP (artifacts exist)", flush=True)
        return
    ...
    for ep in range(MAX_EPOCHS):
        ...
        if val_nll < best_nll - 1e-5:
            ...
            since += 1
        print(f"[k{k} s{seed}] ep {ep:>2} val_nll {val_nll:.6f} lr {opt.param_groups[0]['lr']:.1e}"
    ...
    js.write_text(json.dumps({
        "k": k, "seed": seed, "n_test": int(te.y.shape[0]),
    ...
    print(f"[k{k} s{seed}] DONE test_nll {test_nll:.6f} val_nll {best_nll:.6f} "
...
def main():
    ...
    print(f"=== M27a GPU run: {len(runs)} (window,seed) runs ===", flush=True)
    for i, (k, seed) in enumerate(runs):
        print(f"\n--- [{i+1}/{len(runs)}] k{k} seed{seed} ---", flush=True)
        run_one(k, seed)
