...
4. **Macro covariates remain out of scope** (pipeline decision), with one exception worth its weight: a **rate-incentive proxy derived from the panel itself** — the average `Original Interest Rate` of loans originated in calendar month *t* serves as a market-rate index `mkt_rate(t)`; `incentive_t = current_rate − mkt_rate(t)` is the canonical prepayment driver and the paper's strongest nonlinearity. It uses only contemporaneous data (no leakage). Specified in `01_EDA.md §4` and `02_LOAN_LEVEL.md §2`.
...
- **No external macro covariates** (unemployment, HPI by zip). The incentive proxy partially substitutes for the rate channel; the housing/labor channels are absent — a stated limitation and a clean future-work hook.
