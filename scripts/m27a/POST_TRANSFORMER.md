...
(`src/floan/model/m27a_results_10m/k*_xf_s*.json`) to `origin/main` and writes
`/workspace/dissertation/outputs/XF_DONE.txt` whose last line is `SAFE TO STOP`.

## 1. On the pod (Claude Code) — confirm it finished and pushed
Paste:
...
> origin/main...HEAD` must be `0 0`, and `ls /workspace/repo/src/floan/model/m27a_results_10m/k*_xf_s0.json
...
## 2. On the Mac — pull, then hand to Cowork
```
cd "/Users/felipefritsch/Documents/Masters MCF Oxford/Dissertation/Dissertation - Asset Loans Default Risk"
git pull --ff-only origin main
ls src/floan/model/m27a_results_10m/k*_xf_s0.json | wc -l    # want 11
```
Then tell Cowork **"transformer done"** → it folds the transformer in as the 4th arm of `tab:seqrolling`
